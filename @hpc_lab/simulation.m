function [cpxplot, cpuplot, cpfplot, cpvplot, wlop] = ...
	simulation(obj, ctrl, chip, show)
%SIMULATION Summary of this function goes here
%   Detailed explanation goes here	

	chip.anteSimCheckTM();
	obj.anteSimCheckLab(chip);
	chip.anteSimCheckPM();

	% To Optimize execution, istead of having several functions called and
	%	several ifs statement, we use ctrl.Ts_ctrl as the main time step of
	%	the simulation.
	Nsim = ceil(obj.tsim / ctrl.Ts_ctrl);

	% System:
	% initial offset:
	sys_ts_offset = 0;
	% frequency
	sys_mul = round(ctrl.Ts_ctrl / chip.Ts);
	% output completed
	%ctrl_output_mul = round(obj.ctrl_Texec / obj.Ts);

	% Inputs
	% initial offset:
	target_ts_offset = 0;
	% frequency
	ts_div = obj.Ts_target / ctrl.Ts_ctrl;
	target_mul = floor(ts_div);
	ctrl_mul = floor(1/ts_div) -1;
	ctrl_mul = (ctrl_mul<0)*0 + (ctrl_mul>=0)*ctrl_mul;
	target_counter = target_mul + target_ts_offset;
	target_index = 0;
	% Manage non-integer part:
	% TODO: This does not work well with y.xx when xx> 60 and y = [2,3,4,5] 
	%	because it removes too much. instead with 1, and 6, 7... works well 
	%	I should investigate the math further.
	if target_mul>ctrl_mul
		ntd = (ts_div-target_mul)/target_mul/target_mul;
		nip_sign = -1;
	else
		ntd = (1/ts_div)-(ctrl_mul+1);
		nip_sign = +1;
	end
	nip_mul = ceil(1/ntd);
	nip_counter = nip_mul + target_ts_offset;

	% INIT
	obj.init_compute_model(chip.Ad_true, chip.Bd_true, chip.Nc, chip.vd, chip);

	%TODO
	x = obj.t_init;% + (rand(obj.Ns,1) - 0.5*ones(obj.Ns,1));
	F = chip.F_min*ones(chip.Nc,1);
	V = chip.V_min*ones(chip.vd,1);
	process = ones(chip.Nc,1);
	cpuplot = zeros(Nsim+1,chip.Nc);
	cpxplot = zeros(Nsim+1,chip.Ns);
	cpxplot(1,:) = x;
	cpuplot(1,:) = NaN;
	cpfplot = zeros(Nsim+1,chip.Nc);
	cpvplot = zeros(Nsim+1,chip.vd);
	cpfplot(1,:) = F;
	cpvplot(1,:) = V;
	pwm = 1;
	wl  = [ones(chip.Nc,1) zeros(chip.Nc, chip.ipl -1)];
	pvt{chip.PVT_P} = process;
	pvt{chip.PVT_V} = [];
	pvt{chip.PVT_T} = ctrl.C*x;

	ctrl = ctrl.init_fnc(obj, chip, Nsim);

	% LOOOP
	for s=1:Nsim

		% Input step managing
		target_counter = (target_counter-1)*(target_counter>0) + (target_counter<=0)*(target_mul-1);
		nip_counter = (nip_counter-1)*(nip_counter>0) + (nip_counter<=0)*(nip_mul-1);
		target_index = target_index + (target_counter==target_mul-1) + nip_sign*(nip_counter==nip_mul-1) + ctrl_mul;

		ctrl_pwm = pwm;
		ctrl_wl = wl;
		
		%Compute model:
		index = 1+(s-1)*sys_mul;
		[cpuplot(index+1:index+sys_mul,:), cpxplot(index+1:index+sys_mul,:), wl, pwm, obj] = obj.compute_model(sys_mul, cpxplot(index,:)', V, F, process, chip);	

		% Sim output managing
		T = ctrl.C*cpxplot(index,:)';
		% Noise:
		dim = min(length(T), chip.Nc);
		T = T + [(chip.sensor_noise)*( (rand(dim,1) - 0.5)*2 * chip.sensor_noise_amplitude(chip.PVT_T) ); zeros(length(T)-dim,1)];	

		%if (mod(s-1 + ctrl_ts_offset, ctrl_mul) == 0)
			pvt{chip.PVT_P} = process;
			pvt{chip.PVT_V} = [];
			pvt{chip.PVT_T} = T;
            f_ref = obj.frtrc(min(target_index, size(obj.frtrc,1)),:)';
			pwbdg = obj.tot_pw_budget(min(target_index, length(obj.tot_pw_budget)));
			[F, V, ctrl] = ctrl.ctrl_fnc(f_ref, pwbdg, pvt, ctrl_pwm, ctrl_wl);
		%end

		cpfplot(s+1,:) = F;
		cpvplot(s+1,:) = V;
	end

	cmp = [];
	if obj.compare_vs_baseline
		if obj.pmc_need_update
			warning("[HPC LAB] Error! You are not comparing with the baseline! You should run base_ideal_unr() or checkante!");
		end
		cmp = obj.perf_max_check;
	else
		cmp = size(obj.wltrc,3)-1;
	end
	wlop = obj.wl_index ./ cmp * 100;
	ctrl = ctrl.cleanup_fnc();

	if show
		% Pause because it is bugged on Linux
		pause(0.5);
		obj.xutplot(chip,cpxplot,cpuplot);
		pause(0.5);
		obj.powerconstrplot(chip,cpuplot);
		pause(0.5);
		obj.tempconstrplot(chip,cpxplot);
		pause(0.5);
		obj.perfplot(chip,cpfplot,obj.wl_index);	
		pause(0.5);
		obj.fvplot(chip,cpfplot,cpvplot);

		t1 = chip.Ts*[1:Nsim*sys_mul]';
		t2 = chip.Ts*sys_mul*[1:Nsim]';

		ctrl = ctrl.plot_fnc(t1, t2, cpxplot, cpuplot, cpfplot, cpvplot, wlop);
	end

end

