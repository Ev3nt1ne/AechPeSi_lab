...
% Cores:
chip1.Nc = 36; %72;
chip1.Nh = 6; %9;			% Number of rows
chip1.Nv = 6; %8;			% Number of cols
...
%{
p1 = 94.2399*ones(chip1.Nc,1);
p2 = 10.9303*ones(chip1.Nc,1);
p2(coreid1) = 94.2399;
p2(coreid2) = 68.7267;
...
for wli=2:wl_times
		...
		for di=1:ndom
			...
			switch di
				...
				case 2
					...
					if chip1.Nc == 36
						chip1.VDom([1:3 7:9 13:15], 1) = 1;
						chip1.VDom([4:6 10:12 16:18], 2) = 1;
						chip1.VDom([19:21 25:27 31:33], 3) = 1;
						chip1.VDom([22:24 28:30 34:36], 4) = 1;
					elseif chip1.Nc == 72
						chip1.VDom([1:6 13:18 25:30], 1) = 1;
						chip1.VDom([7:12 19:24 31:36], 2) = 1;
						chip1.VDom([37:42 49:54 61:66], 3) = 1;
						chip1.VDom([43:48 55:60 67:72], 4) = 1;
					else
				...
				case 3
					...
					chip1.VDom = zeros(chip1.Nc, chip1.vd);
					if chip1.Nc == 36
						chip1.VDom([1:2 7:8], 1) = 1;
						chip1.VDom([3:4 9:10], 2) = 1;
						chip1.VDom([5:6 11:12], 3) = 1;
						chip1.VDom([13:14 19:20], 4) = 1;
						chip1.VDom([15:16 21:22], 5) = 1;
						chip1.VDom([17:18 23:24], 6) = 1;
						chip1.VDom([25:26 31:32], 7) = 1;
						chip1.VDom([27:28 33:34], 8) = 1;
						chip1.VDom([29:30 35:36], 9) = 1;
					elseif chip1.Nc == 72
						chip1.VDom([1:4 9:12], 1) = 1;
						chip1.VDom([5:8 13:16], 2) = 1;
						chip1.VDom([17:20 25:28], 3) = 1;
						chip1.VDom([21:24 29:32], 4) = 1;
						chip1.VDom([33:36 41:44], 5) = 1;
						chip1.VDom([37:40 45:48], 6) = 1;
						chip1.VDom([49:52 57:60], 7) = 1;
						chip1.VDom([53:56 61:64], 8) = 1;
						chip1.VDom([65:72], 9) = 1;
					else
						error("didn't set VDom for this Nc!");
					end
				case 4
			...
			if savetofile
				hpc.saveall(simres(1).xop, simres(1).uop, simres(1).fop, simres(1).vop, simres(1).wlop, ...
                    path, regexprep(strcat('Alg: NCP - ', itname), ':', '_'));
			else
				hpc.savetofile(hpc.xutplot(chip1, simres(1).xop, simres(1).uop),...
                    strcat(path,separator_os,regexprep(strcat('Alg: NCP - ', itname), "-", "TP")),1);
			end
	...
	%%		
			...
			if savetofile
				hpc.saveall(simres(1).xop, simres(1).uop, simres(1).fop, simres(1).vop, simres(1).wlop, ...
                    path, regexprep(strcat('Alg: OCC - ', itname), ':', '_'));
			end
